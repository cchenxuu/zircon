...
void IommuImpl::InvalidateIotlbDomainLocked(uint32_t domain_id) {
    DEBUG_ASSERT(lock_.IsHeld());

    // TODO(teisenbe): Read/write draining?
    auto iotlb_invld = reg::IotlbInvalidate::Get(iotlb_reg_offset_).ReadFrom(&mmio_);
    iotlb_invld.set_invld_iotlb(1);
    iotlb_invld.set_invld_request_granularity(reg::IotlbInvalidate::kDomainInvld);
...
void IommuImpl::InvalidateIotlbDomain(uint32_t domain_id) {
    fbl::AutoLock guard(&lock_);
    InvalidateIotlbDomainLocked(domain_id);
}
